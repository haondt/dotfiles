...
M.setup = function()
    ...
    vim.keymap.set('n', '<leader>ow',
        in_normal(function() builtin.diagnostics({ bufnr = 0, severity_bound = vim.diagnostic.severity.WARN }) end),
        { desc = '[o]pen buffer [w]arnings' })
    vim.keymap.set('n', '<leader>oe',
        in_normal(function() builtin.diagnostics({ bufnr = 0, severity_limit = vim.diagnostic.severity.ERROR }) end),
        { desc = '[o]pen buffer [e]rrors' })
